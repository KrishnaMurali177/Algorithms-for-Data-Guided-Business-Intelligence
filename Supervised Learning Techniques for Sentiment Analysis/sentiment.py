...
def feature_vecs_NLP(train_pos, train_neg, test_pos, test_neg):
    ...
    positive_threshold = len(train_pos)*0.01
    negative_threshold = len(train_neg)*0.01
    unique_words = set()
    ...
    for word, f in positive_dict.items():
        if (word not in stopwords) and (f >= positive_threshold) and (f >= 2*negative_dict[word] or negative_dict[word] >= 2*f):
            unique_words.add(word)
    for word, f in negative_dict.items():
        if (word not in stopwords) and (f >= negative_threshold) and (f >= 2*positive_dict[word] or positive_dict[word] >= 2*f):
            unique_words.add(word)
    ...
    train_pos_vec = []
    for review in train_pos:
        temp_dict = {word:0 for word in unique_words}
        for word in review:
            if word in unique_words:
                temp_dict[word] = 1
        train_pos_vec.append(list(temp_dict.values()))
    ...
    for review in train_neg:
        ...
        for word in review:
            if word in unique_words:
                temp_dict[word] = 1
        train_neg_vec.append(list(temp_dict.values()))
    ...
    for review in test_pos:
        ...
        for word in review:
            if word in unique_words:
                temp_dict[word] = 1
        test_pos_vec.append(list(temp_dict.values()))
    ...
    for review in test_neg:
        ...
        for word in review:
            if word in unique_words:
                temp_dict[word] = 1
        test_neg_vec.append(list(temp_dict.values()))
...
def evaluate_model(model, test_pos_vec, test_neg_vec, print_confusion=False):
    ...
    tp = sum(prediction == 'pos')
    fn = sum(prediction == 'neg')

    # Actual Negatives
    prediction = model.predict(test_neg_vec)
    tn = sum(prediction == 'neg')
    fp = sum(prediction == 'pos')
