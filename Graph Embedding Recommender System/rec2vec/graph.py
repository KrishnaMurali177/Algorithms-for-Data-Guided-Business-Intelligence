...
def records_to_graph():
    ...
    # YOUR CODE HERE
    id = 0
    # Creating Nodes for movies and movie ratings
    for m in movies:
        node = Node(id, m, type = 'movie')
        nodedict[m] = node
        nodelist.append(node)
        id += 1
        for r in range(1,6):
            rating_name = m + '_' + str(r)
            node = Node(id, rating_name, type = 'rating')
            nodedict[rating_name] = node
            nodelist.append(node)
            id+= 1

    # Creating Nodes for directors
    for d in directors:
        node = Node(id, d, type = 'director')
        nodedict[d] = node
        nodelist.append(node)
        id+= 1
    ...
    # YOUR CODE HERE
    for u in ratings:
        for m in ratings.get(u):
            r = ratings.get(u).get(m)
            # Filtering out 0 ratings
            if float(r) != 0:
                rating_name = m + '_' + str(r)
                nodedict[rating_name].neighbors.append(nodedict.get(u))
                nodedict[u].neighbors.append(nodedict.get(rating_name))

    # Add edges between movies, directors, actors, genres and movie-rating nodes
    for m in movies:
        d = movies.get(m).director
        # Add edges between movies and directors nodes
        if d:
            nodedict[d].neighbors.append(nodedict.get(m))
            nodedict[m].neighbors.append(nodedict.get(d))
        # Add edge between movies and movie-rating nodes
        for r in range(1, 6):
            rating_name = m + '_' + str(r)
            nodedict[rating_name].neighbors.append(nodedict.get(m))
            nodedict[m].neighbors.append(nodedict.get(rating_name))
        # Add edge between movies and actors nodes
        for a in movies.get(m).actors:
            nodedict[a].neighbors.append(nodedict.get(m))
            nodedict[m].neighbors.append(nodedict.get(a))
        # Add edge between movies and genre nodes
        for g in movies.get(m).genres:
            nodedict[g].neighbors.append(nodedict.get(m))
            nodedict[m].neighbors.append(nodedict.get(g))
