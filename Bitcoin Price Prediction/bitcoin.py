...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0, len(Xi)):
    	y = Xi.iloc[i]
    	sim_fnc = np.exp(wt* similarity_fnc(X.iloc[0: len(X)-1], y.iloc[0:len(y)-1]))
    	num = num + (y[-1]*sim_fnc)
    	den = den + sim_fnc
    return num/den

def similarity_fnc(X,Y):
	"""
	This function computes the similarity function that is used to compute delta
	"""
	m = len(X)
	mean_x = np.mean(X)
	mean_y = np.mean(Y)
	result = 0
	for i in range(0, m):
		result = result + ((X.iloc[i] - mean_x) * (Y.iloc[i] - mean_y))
	return result/(m* np.std(X) * np.std(Y))
...
for i in range(0,len(train1_90.index)) :
  trainDeltaP90 = np.append(trainDeltaP90, computeDelta(weight,test_90.iloc[i],train1_90))
for i in range(0,len(train1_180.index)) :
  trainDeltaP180 = np.append(trainDeltaP180, computeDelta(weight,test_180.iloc[i],train1_180))
for i in range(0,len(train1_360.index)) :
  trainDeltaP360 = np.append(trainDeltaP360, computeDelta(weight,test_360.iloc[i],train1_360))
